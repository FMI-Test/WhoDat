## Logical Segregation for All CoreTech Accounts
- Check Accounts with no DL record in LS report.  
- Commercial distibuted to two input files to avoid Oracle 1000 limit wall.   
...
export COM_INPUT='gect-all-com-1'
...
### II. Get IAM Roles Trust Relationship / Commercial
```bash
# Commercial › Setting › Run from whodat dir
export COM_INPUT_1='gect-all-com-2'
export COM_INPUT_FILE="inputs/${COM_INPUT_1}.tab"
cp "$COM_INPUT_FILE" "${DATAFOLDER}/${COM_INPUT_1}.tab"

./oneidm-explorer.sh --input $COM_INPUT | tee $LOG_BASH
src/iam-list-roles-trust.py --input-file inputs/${COM_INPUT_1}.tab
src/qar_utility.py --datafolder $DATAFOLDER --input-file $COM_INPUT_FILE -cs
```

### III. Get IAM Roles Trust Relationship / GovCloud
```bash
# # GovCloud › Setting › Run from whodat dir
# export AWS_PROFILE=Gov-GEAdmin
# export GOV_INPUT='gect-all-gov'
...
## IV. Extract DL Groups, Groups Managers, Groups Users & Account Users
```bash
...
# 1.2. Env Vars
export COM_FILE="/tmp/oneidm/$COM_INPUT_1-dl-list.json"
export OUT_FILE="/tmp/oneidm/$COM_INPUT_1-LS-Groups-Report.csv"
...
# 3.2. Commercial Users_1
export USER_IN="/tmp/oneidm/$COM_INPUT_1-LS-Credentials-Report.json"
export USER_OT="/tmp/oneidm/$COM_INPUT_1-LS-Users-Report.csv"
src/oneidm-extract.py --input-user $USER_IN --output-file $USER_OT
